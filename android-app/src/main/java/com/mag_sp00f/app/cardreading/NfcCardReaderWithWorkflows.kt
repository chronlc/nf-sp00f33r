...
class NfcCardReaderWithWorkflows(
    ...
    private fun parseRfidiotTlvResponse(response: ByteArray, emvTags: MutableMap<String, String>, context: String) {
        try {
            ...
            parseTlvRecursively(response, 0, response.size, emvTags, context, 0)
            Timber.d("$TAG ✅ $context complete: ${emvTags.size} total tags extracted")
            
        } catch (e: Exception) {
            Timber.e("$TAG ❌ RFIDIOt TLV parsing failed for $context: ${e.message}")
        }
    }
